...
class Eval():
    ...
    def __str__(self, *args, **kwargs):
        return "Eval(mrr=" + str(self.mrr) + ", p@5=" + str(self.p5) + ", r-prec=" + str(self.rprec) + ", map=" + str(
            self.aveprec) + ")"
...
def load_rankings_and_compute_eval(qrelcollection, runreader):  # -> Dict[str, List[WithTruth]] :
    ...
    def eval(sectionId, ranking: List[WithTruth]) -> Eval:
        ...
        ranking = [
            WithTruth(RankElem(elem.elem.sectionId, elem.elem.paraId, r + 1, elem.elem.score, elem.elem.exp_name),
                      elem.is_truth) for r, elem in enumerate(ranking)]
        #     def __init__(self, sectionId, paraId, rank:int, score:float, exp_name):
...
def main():
    ...
    parser.add_argument('qrels', type=argparse.FileType('r'), help='Input path for qrels (ground truth) - probably test200.v2.0/all.test200.cbor.hierarchical.qrels')
